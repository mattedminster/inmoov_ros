    ...
    for (size_t j = 0; j < params.error_blocks.size(); ++j)
    {
      if (params.error_blocks[j].type == "camera3d_to_arm")
      {
        ...
        if (progress_to_stdout)
        {
          ...
          //std::cout << data[i].observations[index].features.size();
          for(size_t hi = 0; hi < data[i].observations[index].features.size(); hi++){
           std::cout << data[i].observations[index].features[hi];
          }
