...
from control_msgs.msg import ExecuteTrajectoryActionGoal
import actionlib
...
def init():
    ...
    rospy.init_node('joint_FollowTrajectory_dispatcher', anonymous=False)
    rate = rospy.Rate(rospy.get_param('/inmoov/bringup/hz')) # 40hz
    print("joint command init at: " + str(rate) + "hz")

    rospy.Subscriber("/execute_trajectory/goal", ExecuteTrajectoryActionGoal, dispatcher)
    ...
    while not rospy.is_shutdown():
        ...
        for j,p in joints.items():
            ...
            try:
                ...
                motorcommand.pin = int(servos[j].servoPin)
