...
def init():
    ...
    for j,b in rospy.get_param('/inmoov/joints').items():
        ...
        busname = '/servobus/' + str(number) + '/motorstatus'
...
def dispatcher(data, bus):
    ...
    try:
        jointname = lookup[((int(bus)*255)+int(data.servoPin))]
        data.joint = jointname
        data.bus = bus
        publisher.publish(data)
    except:
        rospy.logwarn('motor_status_dispatcher:  unknown servo at bus:'+str(bus)+' servo Pin:'+str(data.servoPin))
