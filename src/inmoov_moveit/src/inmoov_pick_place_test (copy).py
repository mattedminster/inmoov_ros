...
class MoveItDemo:
    def __init__(self):
     ...
     # Start the arm in the "resting" pose stored in the SRDF file
      ...
      while len(detected_object.recognized_objects.objects) == 0:
        rospy.sleep(2)
        print("could not find object....trying again")
        detected_object = detectObject() 


      for single_object in detected_object.recognized_objects.objects:
        print(single_object)
        transform = tf_buffer.lookup_transform("base_link",
                                           single_object.pose.header.frame_id, #source frame
        ...
        pose_transformed = tf2_geometry_msgs.do_transform_pose(single_object.pose.pose, transform)
        #print("pose_tranformed")
        #print(pose_transformed)
        pose_x = pose_transformed.pose.position.x
        pose_y = pose_transformed.pose.position.y
        pose_z = pose_transformed.pose.position.z
        ori_x = pose_transformed.pose.orientation.x
        ori_y = pose_transformed.pose.orientation.y
        ori_z = pose_transformed.pose.orientation.z
        ori_w = pose_transformed.pose.orientation.w

      
      print("Detected Object Pose X: " + str(pose_x))
      print("Detected Object Pose Y: " + str(pose_y))
      print("Detected Object Pose Z: " + str(pose_z))
      ...
      target_pose.pose.position.x = pose_transformed.pose.position.x
      target_pose.pose.position.y = pose_transformed.pose.position.y - .065
      target_pose.pose.position.z = pose_transformed.pose.position.z + .05
      ...
      self.gripper_pose_pub.publish(target_pose)
      ...
      ####TEST
      right_arm.set_pose_target(target_pose, right_arm.get_end_effector_link())
      right_arm.go()
      ####TEST
